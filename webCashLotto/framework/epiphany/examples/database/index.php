...
EpiDatabase::employ('mysql','cashlotto','localhost','cashlotto','work4u123'); // type = mysql, database = mysql, host = localhost, user = root, password = [empty]
...
function dbhandler()
{
  $users = getDatabase()->all('SELECT * FROM cl_user');
  ...
  $user = getDatabase()->one("SELECT * FROM cl_user WHERE db_user_id='user1'", array(':Localhost' => 'localhost'));
